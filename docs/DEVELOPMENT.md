...
- Firebase：应用本体使用 BOM 34.3.0，生成项目模板使用 BOM 33.7.0
- 本地库管理与 Maven 依赖解析
- 源代码编辑器（Sora Editor）
- 暗色模式与多语言支持
...
逻辑编辑器是项目中**最复杂的模块**，`LogicEditorActivity.java`（142KB）是全项目最大的文件。

### 核心类关系

```
LogicEditorActivity (142KB)
...
`BlockInterpreter`（15KB）是代码生成的**最底层**，负责将单个 Block 转换为 Java 代码：
...
| 注册表 | 大小 | 内容 |
|--------|------|------|
| `BlockSpecRegistry`（62KB） | ~840 条目 | Block 的**显示规格**：`opCode → spec 字符串`（如 `"set %s to %s"`）|
| `BlockCodeRegistry`（73KB） | ~380 条目 | Block 的**代码模板**：`opCode → Java 代码片段`（如 `"%s = %s"`）|
...
`BlocksHandler.java`（123KB，`mod/hilal/saif/blocks/`）是项目第二大文件（仅次于 `LogicEditorActivity` 142KB），负责扩展 Block 的代码生成：
...
├── 解析流程：
│   ├── 解析 POM 文件获取传递依赖
│   ├── 同坐标重复版本收敛为较新版本，并支持 `+` / 版本区间解析
│   ├── 下载 AAR/JAR 文件
│   │   ├── 主路径写入失败（EPERM / FUSE）时自动切换到 fallback 目录重试
│   │   └── 下载结果写出 `dependency-tree.json` 缓存
│   └── 缓存已下载的依赖
├── 支持的仓库：
│   ├── Maven Central
│   ├── Google Maven
...
│   └── 从其他项目复制
└── 存储路径：
    ├── 主路径：.sketchware/libs/local_libs/
    └── fallback：Android/data/<pkg>/files/local_libs/

LibraryDownloaderDialogFragment (21KB)
├── Maven 依赖下载 UI
├── 输入：groupId:artifactId:version
├── 自动解析传递依赖
├── 共享存储写入遇到 EPERM / FUSE 限制时自动回退到 app-specific 路径
├── 显示下载进度和大小
└── 下载完成后自动添加到本地库列表
...
│
├── libs/
│   └── local_libs/                 # 用户添加的本地库（共享主路径）
│
└── build_cache/                    # 增量构建缓存
    └── {sc_id}/
```

本地库读取时会先查共享目录 `.sketchware/libs/local_libs/`，不存在时再回退到应用专属目录 `Android/data/<pkg>/files/local_libs/`。
...
| 9 | ListView | 19-48 | 见 `ViewBean.java` / `ViewBeans.java` |

## 8.4 加密与备份

### EncryptedFileUtil

项目数据文件通过 `EncryptedFileUtil` 统一读写；启用 `project-data-encryption` 时使用 AES-128-CBC，加密关闭时写入明文，读取时会先尝试解密再回退到明文。

```java
EncryptedFileUtil (9KB)
├── decryptToString(byte[]) → String
│   └── 先尝试 AES-128-CBC 解密，失败时回退为明文 UTF-8
├── encryptString(String) → byte[]
│   └── 仅在开启 `project-data-encryption` 时加密（固定密钥 "sketchwaresecure")
├── writeText(...) / writeBytes(...) → 原子写入
│   ├── 先写入临时文件 (.tmp)
│   ├── 成功后重命名为目标文件
│   └── 失败时保留原文件不变
└── copy / delete / asset 读写等辅助文件操作
```
...
备份恢复：
├── 编辑器内部备份位于 `.sketchware/bak/{sc_id}/`
├── `ProjectDataManager` / `ProjectFileManager` / `LibraryManager` / `ResourceManager` 仅在备份存在、非空且与 `data/` 不同时回退读取
└── `.swb` 导出/导入由 `BackupRestoreManager` / `SingleCopyTask.kt` 负责
```
...
| **修改视图编辑器** | `ViewEditor.java` (52KB), `ViewPane.java` (81KB) | 4.1 |
| **修改逻辑编辑器** | `LogicEditorActivity.java` (142KB) | 4.2 |
...
| 排名 | 文件 | 大小 | 模块 |
|------|------|------|------|
| 1 | `LogicEditorActivity.java` | 142 KB | 逻辑编辑器 |
| 2 | `BlocksHandler.java` | 123 KB | 扩展代码生成 |
| 3 | `ComponentTemplates.java` | 98 KB | 组件模板 |
| 4 | `ViewPane.java` | 81 KB | 视图画布 |
...
| `mod/hilal/saif/` | Hilal Saif | **BlocksHandler（123KB）**、ComponentsHandler、EventsHandler、AppSettings、BlocksManager |
...
| **EncryptedFileUtil** | 项目数据文件的加密/解密工具，支持可选 AES-128-CBC、明文兼容读取与原子写入 | `pro.sketchware.core` |
...
| 文件 | 职责 |
|---|---|
| `EncryptedFileUtil` | 可选 AES-128-CBC 加密、明文兼容读取与原子写入 |
