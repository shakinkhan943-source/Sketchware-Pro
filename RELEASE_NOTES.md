...
- **Jetpack library store — Compose dependencies without a manifest file** — the Compose pipeline gained a modern path beside the ZIP+JSON one: pick a single ZIP in *project libraries → Jetpack Compose* (or in App Settings) whose entries are `zip/<artifact-id>/classes.jar` with the optional `classes.dex`, `res/`, `AndroidManifest.xml`, `proguard.txt`, `assets/` and `libs/*.jar` of an exploded AAR, and the app installs it into the shared store at `.sketchware/libs/JetpackLibs/` while deriving everything else. **Nothing is hand-written any more**: each artifact's dependency list is read off its own class files (constant-pool class references matched against the packages the sibling directories define) and flattened into the `dependency-tree.json` the local library system already understands, a missing `classes.dex` is produced with the embedded D8, and the resource package name for `aapt2 --extra-packages` comes from the artifact's own manifest. Installation runs on one background thread that shuts itself down as soon as its queue drains — an import is a rare burst of I/O, not something worth parking a worker and its buffers for the life of the app on a 2 GB device — and it is cancellable. Because an installed artifact *is* a local library, every project shares the one copy on disk: a project records only the names it activates (`config`, `res/`, `classes.jar`, `classes.dex`, manifest and ProGuard rules all resolve through the store), so enabling Compose in ten projects costs no tenfold storage or dexing. Two guards come with it: the store refuses to install a `kotlin-stdlib`/`kotlinx-coroutines` copy that the app already ships for every Kotlin project (two Kotlin runtimes in one APK crash far from their cause, because the DEX merge keeps only the first definition of each type), and an opt-in override instead lets a **newer** runtime from the ZIP replace the built-in one by comparing versions, so a single coherent copy wins either way. The store also refuses to be fooled by a ZIP whose folders are almost right: a `classes.jar` that is
really a renamed AAR, a jar under the artifact's own name, a `jars/` subfolder, or the whole bundle wrapped
in one extra folder are unpacked and re-keyed at import (and again on **Re-scan**), because the local library
pipeline reads exactly `<id>/classes.jar` and `<id>/classes.dex` — anything else installs happily, lists no
dependencies, and changes nothing when switched on. An artifact that ships only DEX is still wired up (its
edges come from the DEX `type_ids` table) and is now reported as runtime-only instead of silent. App Settings
is store management — import, re-scan, inspect a row's generated `jetpack-info.json` (class, reference and
edge counts plus what had to be repaired) and delete — while activation stays where it belongs, in the
project's own Compose screen, whose list is now reachable even when no legacy bundle is configured and whose
selection is written on leaving the screen. The ZIP+JSON pickers are gone from settings, the build path no
longer asks the legacy manager to extract a package nobody selected (`ComposeBuiltInLibraries
.isBundleAvailable()` guards extraction and artifact selection), and a still-configured legacy bundle keeps
working until the user retires it from that same row.
...
- **D8 aborted with `Should never be called` while rewriting Kotlin metadata** — `:app:mergeExtDexDebug` died inside the R8/D8 8.13.6 that AGP 8.13.0 bundles, because Kotlin 2.4.10 stamps every `@kotlin.Metadata` class with metadata version 2.4.0 and that bundled dexer predates the format: while rewriting the metadata of the external libraries it was dexing it reached a code path it treats as unreachable, which surfaces as `Unexpected error during rewriting of Kotlin metadata ... Should never be called` and fails the affected dex archive. Pinning `com.android.tools:r8` in the version catalog was never a fix, because that artifact is the app's *runtime* library — it is what the on-device `DexCompiler`/`R8Compiler` call to dex and minify user projects — and AGP resolves its dexers from the buildscript classpath instead. The toolchain is now aligned on a combination Google supports (https://developer.android.com/build/kotlin-support lists for Kotlin 2.4: AGP ≥ 8.5.2 — so AGP 8.13.0 and Gradle 8.14.4 stay as they are — plus D8/R8 ≥ 9.1.29): `settings.gradle` overrides AGP's bundled compiler through the documented `pluginManagement { buildscript { classpath "com.android.tools:r8:…" } }` mechanism, the root `build.gradle` additionally places the same artifact ahead of AGP on its own buildscript classpath so newest-wins conflict resolution evicts the 8.13.6 copy from every path the dex tasks can resolve through, and the override version is read from the existing `r8` pin in `gradle/libs.versions.toml` (9.1.31) rather than re-pinned, so the R8 that builds this APK and the R8 that lives inside it are structurally the same artifact — the toml also gained a comment recording the dual role, and a configuration-time floor check now fails the build with an actionable message if the pin is ever moved below 9.1.29 instead of letting the crash return mid-dexing. Nothing is suppressed (no `-dontwarn`, no Kotlin or Sora downgrade): D8 genuinely parses Kotlin 2.4 metadata now.
- **Compose app crashed at launch inside `kotlin.coroutines.CoroutineContext`** — a built Compose APK died the moment a `ComposeView` attached to a window, in `CoroutineContext$DefaultImpls.plus`/`fold` (the signature of two Kotlin runtimes sharing one APK: the DEX merge keeps only the first definition of each type, and code compiled against the copy that lost breaks on the default-method bridges). The runtime override only ever removed the built-in copy when the store's was *strictly newer*, so an imported `kotlin-stdlib`/`kotlinx-coroutines` that was equal or older than the app's built-in (`kotlin-stdlib-2.2.0`, `kotlinx-coroutines-1.8.1`) left **both** copies in the APK. `JetpackLibs.applyRuntimeOverrides()` now guarantees exactly one copy: the newer wins, and when the built-in is the newer one the store's copy is dropped from the project's library list before dexing. Compose projects also now always receive the built-in `kotlinx-coroutines-android` (+ `-core-jvm`) — `AndroidUiDispatcher` and the window recomposer live there — so a Compose project no longer needs the store to carry the runtime just to avoid a `NoClassDefFoundError`.
- **Jetpack store listed every artifact as having no dependencies** — the class-file reader checked the constant-pool magic with `readU4(in) != 0xCAFEBABE`, but `readU4` returns a `long` while `0xCAFEBABE` does not fit in a positive `int`, so the literal sign-extended to `0xFFFFFFFFCAFEBABE` and matched nothing: every class file returned at its first line, `jetpack-info.json` recorded `classes: 1409, references: 0`, and an imported Compose ZIP installed "successfully" with zero dependency edges — which means activating a root added only the root, and the folders its classes actually reference still never reached the classpath. A failure that reports only an absence is the hardest kind to see, so the pool walk now reads each entry whole, indexes into it instead of streaming (an `InputStream.skip` is allowed to skip fewer bytes than asked, and a constant-pool walk that loses alignment does not error, it just reads different things), bounds-checks every read, and writes the counts it gathered next to the graph it derived. Along the way the store also accepts a ZIP that lays an AAR out at the archive root instead of one folder per artifact, and both `jetpack-store.json` and `jetpack-info.json` now list every path a build will read (`classes.jar`, `classes.dex`, `res/`, `config`, …) so "what did it find where" is answerable from the device.
- **Kotlin Activity crash on device (Android's regex engine)** — `KotlinCodeConverter` compiled its patterns from a static initializer with a literal closing brace written unescaped (`\{\s*}`). The desktop JVM accepts that; on Android `Pattern.compileImpl` hands the pattern to ICU4C, which rejects it with `PatternSyntaxException: Syntax error in regexp pattern near index 41` (reported on Android 10), and because it happened during class initialization every Kotlin Activity died with `ExceptionInInitializerError` as soon as the logic editor tried to load the generated source. Every literal brace in the converter is now a one-element character class (`[{]`, `[}]`), the one spelling both engines agree on; that also repairs `new int[n]` / `new String[]{…}` conversion, whose `[^]{}]` class OpenJDK reads as an empty class, and pattern compilation now names the failing expression instead of reporting a bare index.
- **Compose compiler plugin could not be loaded** — building any project with Kotlin sources failed with `ClassNotFoundException: Unable to find class androidx.compose.compiler.plugins.kotlin.ComposePluginRegistrar`. kotlinc discovers a plugin from the `META-INF/services` descriptors of the JAR passed through `-Xplugin`, but the registrar class itself has to come from DEX, and ART cannot define classes out of a JAR's `.class` entries — which is all the copied `files/kt_plugins/*.jar` contains. The plugin is therefore also packaged as an `implementation` dependency so the app's own DEX supplies it to the plugin classloader's parent, and `docs/KOTLIN_COMPILER_IMPROVEMENTS.md` records why both halves are required.
- **Kotlin plugin classpath only carries real plugins** — `kt_plugins` is shared by every project of a Sketchware data folder and receives the plugin's stdlib/annotations JARs beside the plugin itself; all of them used to be handed to `-Xplugin`. JARs that declare no compiler plugin service are skipped now, and a Compose project that ends up without the plugin fails with an actionable message before the compiler runs instead of leaking a raw compiler stack trace.
- **Compose dependency bundle only saw the artifacts a feature named** — the `compose-libs.zip` + `compose-libraries.json` pipeline (project library manager → Jetpack Compose) selected exactly `features[].roots`, and `ComposeArtifact.dependencies` was parsed out of the manifest and read by nobody. Compose splits one library across many artifacts — `androidx.compose.ui:ui` contains no `Color` (`ui-graphics`), no `Dp`/`TextUnit` (`ui-unit`) and no `TextStyle`/`FontWeight` (`ui-text`), `foundation` contains no `Box`/`fillMaxSize` (`foundation-layout`), and `material3` needs `material-ripple` and `material-icons-core` at runtime — so a Compose project died on ~19 `Unresolved reference 'layout'` and `Cannot access class 'Color'` errors from `KotlinCompilerEnhanced` that named neither the cause nor the missing artifact, while the `material3`/`foundation`/`ui` roots themselves resolved fine (that mismatch is what makes the report look random). `ComposeDependencyFeatureResolver` now selects the bundle for what it is: everything packed in the ZIP, minus only the subtree an unselected optional feature owns, with `dependencies` followed transitively as well so a manifest that does declare real edges is pruned precisely. Names are matched against an artifact id, a Maven coordinate with or without its version, or any `_`-folded spelling of either — hand-written bundles mix those — and the walk is cycle-safe, de-duplicated and stays in manifest order so classpath, AAPT2 inputs, APK resources and DEX input agree. This deliberately does *not* rely on the bundle declaring Compose→Compose edges: a manifest generated from "what Gradle had to download additionally" lists only the non-Compose support libraries, and a closure of that graph still misses every artifact named above. An `androidx.compose.*` dependency that no artifact in the package satisfies is now a build error naming the artifact and its requirer, while `lifecycle` / `collection` / `coroutines` / `kotlin-stdlib` edges are logged only, since the app or `android.jar` provides them. The duplicate roots-only selection in `ComposeDependencyManager` is gone (one algorithm for build and library manager), its result is memoized per package hash instead of re-parsing the JSON in every build stage, a DEX-only or JAR-only artifact no longer hands `dexLibraries()`/`addResourcesFromJar()` a non-existent path (a role `resolveClassesJar`/`resolveDex` express as an empty `File` whose absolute path is the working directory), a declared-but-unpacked artifact is reported by id before compilation, shared `--extra-packages` names are emitted once, and `docs/DEVELOPMENT.md` §6.12 records the manifest contract.
