...
- **Compose dependency bundle ignored transitive artifacts** — the `compose-libs.zip` + `compose-libraries.json` pipeline (Settings → Jetpack Compose dependency package) only reached the build the artifacts a feature *named*: `ComposeArtifact.dependencies` was parsed out of the manifest and then read by nothing, while `ProjectBuilder` assumed "the bundle contains all transitive artifacts". Compose splits one library across many artifacts — `androidx.compose.ui:ui` contains no `Color` (`ui-graphics`), no `Dp` (`ui-unit`) and no `TextStyle` (`ui-text`), and `foundation` contains no `Box`/`fillMaxSize` (`foundation-layout`) — so a Kotlin Compose project died on a wall of `Unresolved reference 'layout'` and `Cannot access class 'Color'` errors that named neither the cause nor the missing artifact. `ComposeDependencyFeatureResolver` now expands the feature roots over the declared dependencies (matching an `id`, a Maven coordinate with or without its version, or any `_`-folded spelling of either, so hand-written manifests may mix them), keeps manifest order so classpath, resources and DEX agree, survives dependency cycles, and stops with "X is required by Y but is not declared in the Compose JSON" when a `androidx.compose.*` artifact is genuinely absent — instead of deferring to the compiler — while edges such as `lifecycle`/`collection`/`coroutines`, which the app or `android.jar` provides, are only logged. The duplicated selection copy in `ComposeDependencyManager` is gone (one selection algorithm for build and library manager), its result is memoized per package hash rather than re-read from JSON by every build stage, a DEX-only or JAR-only artifact no longer hands `dexLibraries()`/`addResourcesFromJar()` a non-existent path, a declared-but-unpacked artifact is reported by id before compilation, and `docs/DEVELOPMENT.md` §6.12 records the manifest contract.
